...
# df10 <- df[c('treatment', 'folio', 'ali_score', 'student_gender', 'age', 'studies_mother', 'studies_father', 'fam_income', 'fam_members', 'internet', 'scholarship', 'num_books', 'ali_stdscore', 'mat_score')]
# df10[df10==-9]<-NA
# my_data <- distinct(df10, folio, .keep_all = TRUE)
# 
# 
# nrow(df_T1) + nrow(df_T2)+ nrow(df_T3) +nrow(df_C)
# 
# df_T1 <- filter(df, treatment == 'T1' | treatment == 'C')
# 
# # df1_nc <- filter(df1, cheater == 0)
# # df1_c <- filter(df1, cheater != 0)
# # df1_w <- filter(df1, is.na(omega_final))
# # df1_nw <- filter(df1, omega_final > 0)
# teste <- df %>%
#   group_by(folio) %>%
#   summarize(count=n()) %>%
#   filter(count == 3)
